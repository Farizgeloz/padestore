        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered"  style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="textsize9">{{ $orderku->firstItem() + $loop->index }}</td>
                       
                        <td class=" textsize9">{{ $order->nama_pelanggan }}</td>
                        <td class=" textsize9">{{ $order->metode_bayar }}</td>
                        <td class=" textsize9">{{ $order->jumlah_pesanan }}</td>
                        <td class=" textsize9">{{ $order->sisa_pesanan }}</td>
                        <td class=" textsize9">{{ $order->nama_produk }}</td>
                        <td style="background-color: {{ $order->status === 'Aktif' ? '#0ea314' : '#ffb3b3' }};color: {{ $order->status === 'Aktif' ? '#fff' : '#000' }}">
                            {{ $order->status }}
                        </td>
                        <td class="text-center align-items-center">
                            <!-- dari middleware UserAkses-->
                                ...
                                @endif
                                <button type="button" class="btn btn-primary btn-l" data-bs-toggle="modal" data-bs-target="#exampleModalDetail{{ $order->id_order }}">
                                   <i class="fa fa-eye"></i>
                                </button>
                                @if ($roleuserlogin == "Super Admin" )
                                <a href="/SuperAdmin/Order/Edit/{{$order->id_order}}"
                                    class="btn btn-l btn-warning"><i class="fa fa-pen"></i></a>
                                <button type="button" class="btn btn-danger btn-l" data-bs-toggle="modal" data-bs-target="#exampleModalDelete{{ $order->id_order }}">
...
<!------------------------ Modal Detail----------------------------->
                    <div class="modal fade" id="exampleModalDetail{{ $order->id_order }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-xl w-100" style="">
                            <div class="p-2 modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Detail ({{$order->nomor_kotak}})</h1>
                                    ...
                                    <div class="p-2 row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="m-2 mx-auto row border rounded">
                                                ...
                                                <img src="{{ asset('/storage/kotaks/' . $order->foto) }}" 
                                                onerror="this.src='{{ asset('/storage/assetku/no_image.png')}}';this.onerror='';"
...
<!------------------------ Modal Delete----------------------------->
                    <div class="modal fade" id="exampleModalDelete{{ $order->id_order }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-l">
                            <form action="{{ route('folder_order.destroy', $order->id_order) }}" method="POST" class="p-2 border border-2 rounded border-success">
                                @csrf
                                @method('DELETE')
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="exampleModalLabel">Hapus Data ({{$order->nama_pelanggan}})</h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
        ...
        <div class="d-flex custom-pagination justify-content-end">
            {{ $orderku->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
@endsection